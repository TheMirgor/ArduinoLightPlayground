...
void loop(void) {
  ...
  int bounce = 70;  //map(analogRead(POT_R), 0, 1024, 75, 125);
  if (pattern == 1) {
    knightRiderEffect();
    if (mIrisLoc > NUMPIXELS || mIrisLoc < 0 || value > bounce) {
      mStepper = -mStepper;
    }
    mIrisLoc += mStepper;
  } else if (pattern == 2) {
    ...
    showFromCenter(ColorWheel(value));
  }  else {
    fireAndIceEffect();
...
// strip.Color(value * baseRed / 255, value * baseGreen / 255, value * baseBlue / 255)
uint32_t ColorWheel(byte WheelPos) {
  if (WheelPos < 15) {
    return strip.Color(0,0,0);
  } else if (WheelPos < flareThreshold) {
    WheelPos -= flareThreshold;
    return strip.Color(WheelPos * baseRed / 255, WheelPos * baseGreen / 255, WheelPos * baseBlue / 255);
  } else {
    WheelPos -= 40;
